...
async def get_team_attendance_service(db,manager_id,start_date,end_date):
        try:
            log_data = select(LogAttendance).join(Employee).where((Employee.manager_id == manager_id) & (func.date(LogAttendance.log_in_time).between(start_date,end_date)))
            result = await db.execute(log_data)
            today_log = result.scalars().all()
            return today_log
